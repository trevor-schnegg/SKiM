...
fn main() {
    ...
    let mut output_writer = BufWriter::new(create_output_file(output_loc_path, "r2l"));

    let fastq_reads_iter = get_fastq_iter_of_file(reads_path);

    info!("looping through reads at {:?}", reads_path);

    let read2length = fastq_reads_iter
        .into_iter()
        .filter_map(|record_result| match record_result {
            Ok(read) => Some((read.id().to_string(), read.seq().len())),
            Err(e) => {
    ...
    for (readid, length) in read2length {
        output_writer
            .write(format!("{}\t{}\n", readid, length).as_bytes())
